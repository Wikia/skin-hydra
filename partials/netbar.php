...
<div id="netbar">
	<div class="netbar-flex">
		...
		<div class="netbar-box left"><a href="http://support.gamepedia.com/"><?= wfMessage('netbar-help')->text() ?></a></div>
		<?php } else { ?>
		...
		<div class="netbar-box left officialwiki"><a href="/index.php?title=Special:AllSites&amp;filter=official"><img src="/skins/Hydra/images/netbar/official-wiki.svg"></a></div>
		<?php } ?>
		<?php
		...
		*/
		...
		?>
		<?php if ($wgUser->isAnon()) { ?>
		<div class="netbar-box right"><a href="<?= $personalTools['login']['links'][0]['href'] ?>" id="login-link" class="gold-link"><?= wfMessage('netbar-signin')->text() ?></a></div>
			<?php if ($wgUser->isAllowed( 'createaccount' )) { ?>
		<div class="netbar-box right"><a href="<?= $personalTools['createaccount']['links'][0]['href'] ?>" id="register-link" class="gold-link"><?= wfMessage('netbar-register')->text() ?></a></div>
			<?php } ?>
		<?php } else { ?>
		<?php if (!$wgUser->isAnon() && isset($personalTools['notifications-alert'])) {
			if (isset($personalTools['notifications-alert']['class'])) {
				$personalTools['notifications-alert']['class'] .= ' netbar-box right';
			} else {
				$personalTools['notifications-alert']['class'] = 'netbar-box right';
			}
			echo $skin->makeListItem('notifications-alert', $personalTools['notifications-alert'], ['tag' => 'div']);
			unset($personalTools['notifications-alert']);
		} ?>
		<?php if (!$wgUser->isAnon() && isset($personalTools['notifications-notice'])) {
			if (isset($personalTools['notifications-notice']['class'])) {
				$personalTools['notifications-notice']['class'] .= ' netbar-box right';
			} else {
				$personalTools['notifications-notice']['class'] = 'netbar-box right';
			}
			echo $skin->makeListItem('notifications-notice', $personalTools['notifications-notice'], ['tag' => 'div']);
